...
final class MFoodGroupDedupeTests: TestBase {
    ...
    func testDifferentGroupRaw() throws {
        let c1 = MCategory.create(testContext, userOrder: 10, archiveID: catArchiveID1, createdAt: date0)
        let s1 = MFoodGroup.create(testContext, category: c1, userOrder: 4, groupRaw: groupRaw1, createdAt: date1)
        let s2 = MFoodGroup.create(testContext, category: c1, userOrder: 8, groupRaw: groupRaw2, createdAt: date2)
        try testContext.save() // needed for fetch request to work properly

        try MFoodGroup.dedupe(testContext, categoryArchiveID: catArchiveID1, groupRaw: groupRaw1)
    ...
    func testSameGroupRawWithinCategory() throws {
        let c1 = MCategory.create(testContext, userOrder: 10, archiveID: catArchiveID1, createdAt: date0)
        let s1 = MFoodGroup.create(testContext, category: c1, userOrder: 4, groupRaw: groupRaw1, createdAt: date1)
        let s2 = MFoodGroup.create(testContext, category: c1, userOrder: 8, groupRaw: groupRaw1, createdAt: date2)
        try testContext.save() // needed for fetch request to work properly
    ...
    func testSameGroupRawOutsideCategory() throws {
        ...
        let s1 = MFoodGroup.create(testContext, category: c1, userOrder: 4, groupRaw: groupRaw1, createdAt: date1)
        let s2 = MFoodGroup.create(testContext, category: c2, userOrder: 8, groupRaw: groupRaw1, createdAt: date2)
        try testContext.save() // needed for fetch request to work properly
