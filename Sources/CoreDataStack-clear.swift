...
extension CoreDataStack {
    ...
    func clearPrimaryEntities(_ context: NSManagedObjectContext) throws {
        try context.deleter(MServing.self)
        try context.deleter(MFoodGroup.self)
        try context.deleter(MCategory.self)
    ...
    public func clearZEntities(_ context: NSManagedObjectContext, inStore: NSPersistentStore? = nil) throws {
        try context.deleter(ZServingRun.self, inStore: inStore)
        try context.deleter(ZDayRun.self, inStore: inStore)
        try context.deleter(ZServing.self, inStore: inStore)
        try context.deleter(ZCategory.self, inStore: inStore)
    }
}
